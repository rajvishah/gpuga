...
FLAG gaEvolve1DBINPopulation(BIN1D **popAdd,GAContext *ga,GNMContext *genome,GAStats *stats,UDATA *udata)
{
	...
	struct timeval tv1;
	struct timezone tz1;
	unsigned long start_seconds,cpu_start_seconds;
	unsigned long start_micro_seconds,cpu_start_useconds;
	unsigned long end_seconds,cpu_end_seconds;
	unsigned long end_micro_seconds,cpu_end_useconds;
